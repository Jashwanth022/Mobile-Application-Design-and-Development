...
class WeekActivity : AppCompatActivity() {
    ...
    private lateinit var tvTopStockName: android.widget.TextView
    private lateinit var tvTopStockChange: android.widget.TextView
    private lateinit var tvTopStockAccuracy: android.widget.TextView
    private lateinit var api: ApiService

    private var allStocks = listOf<Stock>()
    private var filteredStocks = allStocks.toList()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvTopStockName = findViewById(R.id.tvTopStockName)
        tvTopStockChange = findViewById(R.id.tvTopStockChange)
        tvTopStockAccuracy = findViewById(R.id.tvTopStockAccuracy)
    ...
    private fun fetchData() {
        api.getPredictions().enqueue(object : Callback<List<StockResponse>> {
            override fun onResponse(
                ...
                if (data != null) {
                    val sortedData = data.sortedByDescending { it.next_week }
                    allStocks = sortedData.map {
                        Stock(it.stock, it.cap, it.next_week, it.accuracy.toInt())
                    }
                    filteredStocks = allStocks
                    setupRecyclerView()

                    val topStock = sortedData.firstOrNull()
                    topStock?.let {
                        tvTopStockName.text = it.stock
                        tvTopStockChange.text = if (it.next_week >= 0) "+${it.next_week}%" else "${it.next_week}%"
                        tvTopStockAccuracy.text = "${it.accuracy.toInt()}% Accuracy"
                    }
                }
            }

            override fun onFailure(call: Call<List<StockResponse>>, t: Throwable) {
                Toast.makeText(this@WeekActivity, "Failed to fetch data", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun setupChart() {
        ...
        val dummyPrices = listOf(158.0, 162.0, 165.5, 163.0, 168.2, 172.0, 175.0)
        
        dummyPrices.forEachIndexed { index, price ->
            entries.add(Entry(index.toFloat(), price.toFloat()))
        }

        val dataSet = LineDataSet(entries, "Weekly Trend")
