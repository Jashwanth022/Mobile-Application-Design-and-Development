...
class DayActivity : AppCompatActivity() {
    ...
    private fun fetchData() {
        api.getPredictions().enqueue(object : Callback<List<StockResponse>> {
            override fun onResponse(
                ...
                if (data != null) {
                    val sortedData = data.sortedByDescending { it.next_day }
                    allStocks = sortedData.map {
                        Stock(it.stock, it.cap, it.next_day, it.accuracy.toInt())
                    }
                    ...
                    topStock?.let {
                        ...
                        tvTopStockChange.text = if (it.next_day >= 0) "+${it.next_day}%" else "${it.next_day}%"
            ...
            override fun onFailure(call: Call<List<StockResponse>>, t: Throwable) {
                Toast.makeText(this@DayActivity, "Failed to fetch data", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun setupChart() {
        ...
        val dummyPrices = listOf(150.0, 152.5, 151.0, 155.2, 158.0, 157.5, 160.0)
