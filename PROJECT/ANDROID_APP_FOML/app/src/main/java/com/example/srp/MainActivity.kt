...
class MainActivity : AppCompatActivity() {
        ...
        ApiService.create().getPredictions().enqueue(object : Callback<List<StockResponse>> {
            override fun onResponse(call: Call<List<StockResponse>>, response: Response<List<StockResponse>>) {
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@MainActivity, "Failed to load top picks", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    private fun updateTopPick(data: List<StockResponse>) {
        ...
        topStock?.let { stock ->
            tvName.text = "\uD83D\uDD25 Top Pick: ${stock.stock}"
            tvReturn.text = "Expected Return: +${stock.next_day}%"
            
            findViewById<View>(R.id.topPickCard).setOnClickListener {
                openDetail(stock.stock, "+${stock.next_day}%", "${stock.accuracy}%")
            }
        }
    }
